...
- 📺 **内置随身小电视**：贴个 B 站链接就能边写代码边看剧；还能一键换成假广告弹窗皮肤，伪装成没关掉的弹窗广告
...
**广告模式**（底部「📢 伪装广告」按钮）：窗口换个假推广弹窗皮肤钉死在屏幕右下角——白边框、假头条（「输入法 · 每日精选推荐」+「广告」角标）、假脚条（「不再提示」），视频照常播放。假边框上的「✕」「不再提示」都真的关掉小电视，**双击假标题栏**才退回普通窗口。「⤢/⤡」在 4 档视频尺寸（400×225 ~ 640×360）之间切换，选中的档位记在 localStorage。
...
└── client/
    ...
    └── WhaleTv.tsx     B站小电视悬浮窗（链接解析 + iframe 播放 + 拖动 + 最近记录 + 广告模式假边框）
assets/blubby/
